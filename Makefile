...
test-openmetadata:
	@OPENMETADATA_ADMIN_PASSWORD="$(OPENMETADATA_ADMIN_PASSWORD)" python -m pytest tests/integration/openmetadata

test-table-route:
	RUN_TABLE_ROUTE_INTEGRATION=1 KIND_CLUSTER_NAME=$(KIND_CLUSTER_NAME) LAKEFS_NAMESPACE=$(LAKEFS_NAMESPACE) LAKEFS_PORT=$(LAKEFS_PORT) python -m pytest tests/integration/table_route

e2e-data-reproducibility:
	RUN_DATA_REPRODUCIBILITY_E2E=1 KIND_CLUSTER_NAME=$(KIND_CLUSTER_NAME) LAKEFS_NAMESPACE=$(LAKEFS_NAMESPACE) LAKEFS_PORT=$(LAKEFS_PORT) python scripts/phase_03/data_reproducibility.py
	RUN_DATA_REPRODUCIBILITY_E2E=1 python -m pytest tests/e2e/phase_03
...
apply-openmetadata: apply-keycloak apply-postgres
	...
	kubectl --context kind-$(KIND_CLUSTER_NAME) rollout restart --namespace $(OPENMETADATA_NAMESPACE) deployment/openmetadata-oidc-discovery-adapter
	...
	@if command -v helm >/dev/null; then helm --kube-context kind-$(KIND_CLUSTER_NAME) upgrade --install $(OPENMETADATA_DEPENDENCIES_RELEASE) openmetadata-dependencies --repo $(OPENMETADATA_CHART_REPO) --version $(OPENMETADATA_DEPENDENCIES_CHART_VERSION) --namespace $(OPENMETADATA_NAMESPACE) --create-namespace --values platform/charts/openmetadata/values-dependencies-dev-kind.yaml --wait --timeout 10m --history-max 3; else if [ ! -f "$(KUBECONFIG)" ]; then echo "helm is not installed and KUBECONFIG does not point to a readable file: $(KUBECONFIG)"; exit 127; fi; docker run --rm --network host -v "$(KUBECONFIG):/root/.kube/config:ro" -v "$(CURDIR):/workspace" -w /workspace "$(HELM_RUNNER_IMAGE)" --kube-context kind-$(KIND_CLUSTER_NAME) upgrade --install $(OPENMETADATA_DEPENDENCIES_RELEASE) openmetadata-dependencies --repo $(OPENMETADATA_CHART_REPO) --version $(OPENMETADATA_DEPENDENCIES_CHART_VERSION) --namespace $(OPENMETADATA_NAMESPACE) --create-namespace --values platform/charts/openmetadata/values-dependencies-dev-kind.yaml --wait --timeout 10m --history-max 3; fi
	@if command -v helm >/dev/null; then helm --kube-context kind-$(KIND_CLUSTER_NAME) upgrade --install $(OPENMETADATA_RELEASE) openmetadata --repo $(OPENMETADATA_CHART_REPO) --version $(OPENMETADATA_CHART_VERSION) --namespace $(OPENMETADATA_NAMESPACE) --create-namespace --values platform/charts/openmetadata/values-dev-kind.yaml --wait --timeout 10m --history-max 3; else if [ ! -f "$(KUBECONFIG)" ]; then echo "helm is not installed and KUBECONFIG does not point to a readable file: $(KUBECONFIG)"; exit 127; fi; docker run --rm --network host -v "$(KUBECONFIG):/root/.kube/config:ro" -v "$(CURDIR):/workspace" -w /workspace "$(HELM_RUNNER_IMAGE)" --kube-context kind-$(KIND_CLUSTER_NAME) upgrade --install $(OPENMETADATA_RELEASE) openmetadata --repo $(OPENMETADATA_CHART_REPO) --version $(OPENMETADATA_CHART_VERSION) --namespace $(OPENMETADATA_NAMESPACE) --create-namespace --values platform/charts/openmetadata/values-dev-kind.yaml --wait --timeout 10m --history-max 3; fi
